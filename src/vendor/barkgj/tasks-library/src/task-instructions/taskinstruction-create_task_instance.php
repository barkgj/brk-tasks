	...
	{	
		function execute($taskid, $taskinstanceid, $then_that_item)
		{
			...
			if ($state == "STARTED")
			{
				...
				{
					$m = "brk_tasks_instance_do_create_task_instance; create_taskid still has unreplaced placeholders; $create_taskid";
					if (tasks::isheadless())
					{
				...
				else
				{
					...
					if (true) // $should_render_required_fields)
					{
						...
						if (functions::stringcontains($happyflow_behaviour, "start_child_task_instance"))
						{
							...
							// 
							$assigned_to = brk_tasks_gui_getemployeeidcurrentuser();
