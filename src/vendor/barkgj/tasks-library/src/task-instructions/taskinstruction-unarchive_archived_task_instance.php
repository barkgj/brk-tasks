...
function brk_tasks_instance_do_unarchive_archived_task_instance($then_that_item, $taskid, $taskinstanceid)
{
  ...
  $check_instancemeta = brk_tasks_getinstance($taskid_to_unarchive, $taskinstanceid_to_unarchive);
  if ($check_instancemeta["isfound"] == false)
  {
	  ...
	  $findarchivedinstanceresult = brk_tasks_archive_findarchivedinstance($taskid_to_unarchive, $taskinstanceid_to_unarchive);
	  if ($findarchivedinstanceresult["isfound"])
	  {
	  	$result["console"][] = "FOUND IN ARCHIVE";
	  	
	  	$unarchive_result = brk_tasks_unarchive_archived_task_instance($taskid_to_unarchive, $taskinstanceid_to_unarchive);
